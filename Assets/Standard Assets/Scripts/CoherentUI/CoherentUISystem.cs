...
public class CoherentUISystem : MonoBehaviour {
		...
		bool invertY)
	{
		...
		if (!isOnSurface)
		{
			camWidth = view.gameObject.GetComponent<Camera>().pixelWidth;
			camHeight = view.gameObject.GetComponent<Camera>().pixelHeight;
		}
		else
		{
			...
			{
				camWidth = surfaceCameraObj.GetComponent<Camera>().pixelWidth;
				camHeight = surfaceCameraObj.GetComponent<Camera>().pixelHeight;
			}
			else
			{
				return;
